...
class ballon() :
	...
	def setVitAng(self, vit1) :
		...
		duree = time.time()
		dirBasex, dirBasey = self.direction
		v = (vit_ballon*dirBasex*duree, vit_ballon*dirBasey*duree)
		vit_norme = max(0, norme(v)-(0.001*norme(v)**2-0.06*norme(v)))
		self.setVitAngD(vit_norme)
		self.setVitAngG(vit_norme)
		self.logger.debug("Vitesse globale set à %d (%s)", vit_norme, self.nom)
